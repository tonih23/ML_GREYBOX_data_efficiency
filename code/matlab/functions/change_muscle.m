...
for i = 0:param.numMuscles-1
    ...
    fibre_length = m_child.getFiberLength(state);

    tendon_length = m_child.getTendonLength(state)*0.6;
    % tendon_length = muscle_length-fibre_length;

    m.setTendonSlackLength(tendon_length);
    m.setOptimalFiberLength(fibre_length*2);

    m.set_appliesForce(1);
    m.set_ignore_activation_dynamics(0)
    m.set_ignore_tendon_compliance(0)

    m.set_max_isometric_force(m.get_max_isometric_force/5);
    m.set_max_contraction_velocity(1);
    m.set_max_control(0.1);
    m.set_min_control(0.01);

    % m_child.setTendonSlackLength(tendon_length);
    m_child.setOptimalFiberLength(fibre_length);
    m_child.set_default_fiber_length(fibre_length)
    m_child.set_fiber_damping(1);

    m_child.setDefaultActivation(0);
    % 
    % if strcmp(m.getName, "FDPI")
    %     m_child.setDefaultActivation(1);
    %     m.set_appliesForce(0.01);
    % end
